...
// + sighash (72WU), for a total of 105 WU.
...
fn test_taproot_script_spend_sign_include_some_leaves() {
    ...
    assert!(psbt.inputs[0].tap_script_sigs.iter().all(|s| {
        included_script_leaves.contains(&s.0 .1) && !excluded_script_leaves.contains(&s.0 .1)
    }));
}
